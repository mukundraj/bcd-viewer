...
function Loader({dataConfig}){
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      let coordsUrl = `${basePath}${relativePath}/gene_exprs_cshl/puck${chosenPuckid.pid}/coords.csv`
    ...
    const fetchNissl = async () => {
      ...
      let nis_url = `https://storage.googleapis.com/bcdportaldata/genexp_data/gene_exprs_cshl/puck${chosenPuckid.pid}/nis_${pad(chosenPuckid.pid, 3)}.png`
    ...
    const fetchAtlas = async () => {
      // todo: remove hardcoding below by updating basePath and relativePath
      let atlas_url = `https://storage.googleapis.com/bcdportaldata/genexp_data/gene_exprs_cshl/puck${chosenPuckid.pid}/chuck_sp_wireframe_${pad(chosenPuckid.pid,3)}.png`;
    ...
    const fetchGeneOptions = async () => {
      let geneOptionsUrl = `${basePath}${relativePath}/gene_exprs_cshl/puck${chosenPuckid.pid}/geneOptions.json`
  ...
  useEffect(()=>{
    ...
    // read gene data
    const fetchData = async () => {
      let geneDataUrl = `${basePath}${relativePath}/gene_exprs_cshl/puck${chosenPuckid.pid}/${prefix}${chosenGene[0]}.csv`
      ...
      if (chosenGene2.length > 0){ // fetch and update both geneData1 and geneData2
        ...
        let geneDataUrl = `${basePath}${relativePath}/gene_exprs_cshl/puck${chosenPuckid.pid}/${prefix}${chosenGene2[0]}.csv`
        // let geneDataUrl = await getUrl(geneDataPath);

        // load metadata for gene1 and gene2
        let metaDataUrl1 = `${basePath}${relativePath}/gene_exprs_cshl/puck${chosenPuckid.pid}/metadata_gene_${chosenGene[0]}.json`
        // let metaDataUrl1 = await getUrl(meta_data_path1);
        let metaDataUrl2 = `${basePath}${relativePath}/gene_exprs_cshl/puck${chosenPuckid.pid}/metadata_gene_${chosenGene2[0]}.json`
  ...
  useEffect(()=>{
    if (geneOptions.includes(chosenGene2[0])){
      ...
      // read gene data
      const fetchData = async () => {
        let gene2DataUrl = `${basePath}${relativePath}/gene_exprs_cshl/puck${chosenPuckid.pid}/${prefix}${chosenGene2[0]}.csv`
