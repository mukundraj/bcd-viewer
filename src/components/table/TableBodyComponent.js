            ...
            <tr key={data.id}>
              {columns.map(({ accessor }) => {
                ...
                return <td key={accessor}>
                  {isNaN(tData)?
                    <><button className="btn btn-light btn-sm py-0" disabled={data.st=='Y'?false:true} style={{borderWidth:"0"}} onClick={()=>{toCellSpatial(data[accessor])}}>{data[accessor]}</button>, <span style={{color:'#8E44AD'}}>{data.cc}</span>, <span style={{color:'#BB8FCE'}}>{data.pct}%</span></>:tData===0?"-":""}
